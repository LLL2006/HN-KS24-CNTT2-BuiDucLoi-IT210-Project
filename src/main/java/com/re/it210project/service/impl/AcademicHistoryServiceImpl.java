...
public class AcademicHistoryServiceImpl implements AcademicHistoryService {
    ...
    public List<AcademicHistoryResponse> getStudentHistory(Long studentId) {
        List<MentoringSession> sessions = mentoringSessionRepository.findByStudentIdOrderByStartTimeDesc(studentId);
    ...
    private AcademicHistoryResponse toHistoryResponse(MentoringSession session) {
        AcademicEvaluation evaluation = academicEvaluationRepository.findBySessionId(session.getId()).orElse(null);
        BorrowingRecord borrowingRecord = borrowingRecordRepository.findBySessionId(session.getId()).orElse(null);
        ...
        if (borrowingRecord != null && borrowingRecord.getDetails() != null) {
            equipments = borrowingRecord.getDetails().stream()
                    .map(this::toBorrowedEquipment)
                    .toList();
        }

        String lecturerName = session.getLecturer().getUser().getProfile().getFullName();
        String departmentName = session.getLecturer().getDepartment().getName();
